/**
 * Manage Categories - 与移动端 app/categories-manage.tsx 一一对应，复用 shared lib
 */
import { useState, useEffect } from 'react';
import { getCategories, createCategory, updateCategory, deleteCategory } from '../lib/shared/index';
...
export default function CategoriesManage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      const item = await createCategory(newName.trim(), newColor);
          ...
          <div className="manage-list-colors">
            {COLOR_OPTIONS.map((c) => (
              <button
                key={c}
                type="button"
                className="manage-list-color-dot"
                style={{ background: c }}
